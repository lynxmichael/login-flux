...
/* ============================
    TRANSACTIONS (dernières 10) - CORRIGÉ
   ============================ */
$transactions_stmt = $pdo->prepare("
    SELECT 
        t.*,
        COALESCE(t.stock_symbol, '') as stock_symbol,
        COALESCE(t.quantity, 0) as quantity,
        COALESCE(t.price, 0) as price,
        COALESCE(t.amount, t.quantity * t.price) as amount,
        CASE 
            WHEN t.type IN ('buy', 'sell') AND t.quantity > 0 AND t.price > 0 
...
// DEBUG : afficher les transactions dans le code HTML (commentaire)
echo "<!-- DEBUG Transactions for user $user_id: ";
var_dump($transactions);
echo " -->";

/* ============================
   ACTIFS (positions en cours) - CORRIGÉ
   ============================ */
        ...
        <header class="dashboard-header">
            <div class="dashboard-header-top">
                ...
                <div class="dashboard-actions">
                    ...
                    <button class="dashboard-action-btn refresh-btn" onclick="openLogoutModal()">
                        <span>🚪</span>
                        Déconnexion
                    </button>
                </div>
            ...
            <div id="logoutModal" class="modal" style="display: none;">
                <div class="modal-content logout-modal">
                    ...
                    <div class="modal-buttons">
                        <button type="button" class="btn-cancel" onclick="closeModal('logoutModal')">
                            Annuler
                        </button>
...
<div class="dashboard-widget">
    ...
    <div class="transactions-widget">
                ...
                <div class="transaction-item">
                    <div class="transaction-info">
                        <div class="transaction-type <?php echo $transaction['type']; ?>">
                            ...
                            echo $typeLabels[$transaction['type']] ?? strtoupper($transaction['type']);
                            ?>
                        </div>
                        <div class="transaction-details">
                            <?php 
                            // Afficher le symbole pour les transactions buy/sell
                            if (in_array($transaction['type'], ['buy', 'sell']) && !empty($transaction['stock_symbol'])): 
                            ?>
                                <strong class="stock-symbol"><?php echo htmlspecialchars($transaction['stock_symbol']); ?></strong>
                                <?php if ($transaction['quantity'] > 0): ?>
                                    <span class="quantity-badge"><?php echo $transaction['quantity']; ?> actions</span>
                                    <?php if ($transaction['price'] > 0): ?>
                                        <span class="price-badge">à <?php echo number_format($transaction['price'], 0, ',', ' '); ?> FCFA</span>
                                    <?php endif; ?>
                                <?php endif; ?>
                            ...
                            <span class="transaction-date">
                                <?php echo date('d/m/Y à H:i', strtotime($transaction['created_at'])); ?>
                            </span>
                        </div>
                                            </div>
                    <div class="transaction-amount <?php echo (in_array($transaction['type'], ['buy', 'withdraw', 'withdrawal', 'transfer', 'fee'])) ? 'negative' : 'positive'; ?>">
                        <?php 
                        $amount = abs(floatval($transaction['amount']));
                        $sign = (in_array($transaction['type'], ['buy', 'withdraw', 'withdrawal', 'transfer', 'fee'])) ? '-' : '+';
                        echo $sign . number_format($amount, 0, ',', ' '); 
                        ?> FCFA
                    </div>
                </div>
            <?php endforeach; ?>
        <?php endif; ?>
            ...
            <!-- Actifs en portefeuille - CORRIGÉ -->
            <div class="dashboard-widget">
                <div class="widget-header">
                    <h3>Mes Actifs</h3>
                            ...
                            <div class="asset-item">
                                <div class="asset-info">
                                    <div class="asset-symbol">
                                        <?php echo htmlspecialchars($asset['stock_symbol']); ?>
                                        <span class="asset-name"><?php echo htmlspecialchars($asset['stock_name']); ?></span>
                                    </div>
                                    <div class="asset-details-row">
                                        <div class="asset-quantity">
                                            <?php echo number_format($asset['total_quantity'], 0, ',', ' '); ?> actions
                                        </div>
                                        <div class="asset-prices">
                                            <span class="price-label">Prix moyen:</span>
                                            <span class="price-value"><?php echo number_format($asset['avg_price'], 0, ',', ' '); ?> FCFA</span>
                                            <span class="price-separator">|</span>
                                            <span class="price-label">Prix actuel:</span>
                                            <span class="price-value"><?php echo number_format($asset['current_price'], 0, ',', ' '); ?> FCFA</span>
                                        </div>
                                    </div>
                                </div>
                                <div class="asset-value">
                                    <div class="asset-total-value">
                                        <?php echo number_format($asset['total_value'], 0, ',', ' '); ?> FCFA
                                    </div>
                                    <?php if ($asset['avg_price'] > 0): ?>
                                        <div class="asset-change <?php echo ($asset['profit_loss_percent'] >= 0) ? 'positive' : 'negative'; ?>">
                                            <?php 
                                            echo ($asset['profit_loss_percent'] >= 0 ? '+' : '') . number_format($asset['profit_loss_percent'], 2); 
                                            ?>%
                                        </div>
                                    <?php endif; ?>
                                </div>
                ...
                <div class="widget-header">
                    ...
                    <div class="notifications-actions">
                        ...
                        <?php if (!empty($notifications)): ?>
                            <button class="mark-read-btn" onclick="markAllNotificationsAsRead()">Tout marquer comme lu</button>
                        <?php endif; ?>
                    </div>
                </div>
                <div class="notifications-widget">
                    <?php if (empty($notifications)): ?>
                        <div class="loading-data">Aucune nouvelle notification</div>
                    <?php else: ?>
                        ...
                        <?php foreach ($transactions as $transaction): ?>
                            <tr>
                                ...
                                <td>
                                    <?php 
                                    if (in_array($transaction['type'], ['buy', 'sell']) && !empty($transaction['stock_symbol'])) {
                                        echo htmlspecialchars($transaction['stock_symbol']);
                                    } else {
                                        echo '-';
                                    }
                                    ?>
                                ...
                                <td>
                                    <?php 
                                    if (in_array($transaction['type'], ['buy', 'sell']) && $transaction['quantity'] > 0) {
                                        echo number_format($transaction['quantity'], 0, ',', ' ');
                                    } else {
                                        echo '-';
                                    }
                                    ?>
                                ...
                                <td>
                                    <?php 
                                    if (in_array($transaction['type'], ['buy', 'sell']) && $transaction['price'] > 0) {
                                        echo number_format($transaction['price'], 0, ',', ' ') . ' FCFA';
                                    } else {
                                        echo '-';
                                    }
                                    ?>
                                </td>
                                <td class="<?php echo (in_array($transaction['type'], ['buy', 'withdraw', 'withdrawal', 'transfer', 'fee'])) ? 'negative' : 'positive'; ?>">
                                    <?php 
                                    $amount = 0;
                                    if (isset($transaction['amount']) && $transaction['amount'] != 0) {
                                        $amount = abs($transaction['amount']);
                                    } elseif (isset($transaction['quantity']) && isset($transaction['price']) && $transaction['quantity'] > 0 && $transaction['price'] > 0) {
...
function confirmLogout() {
    ...
    // Rediriger vers la page de déconnexion après un délai
    setTimeout(() => {
        window.location.href = 'logout.php';
    }, 1000);
}

// Ajoutez cette fonction utilitaire pour fermer les modals
function closeModal(modalId) {
    const modal = document.getElementById(modalId);
    modal.classList.remove('show');
    setTimeout(() => {
...
function markNotificationAsRead(notificationId) {
    ...
    .then(data => {
        if (data.success) {
            // Retirer le style "unread" et mettre à jour le compteur
...
function markAllNotificationsAsRead() {
    ...
    .then(data => {
        if (data.success) {
            // Retirer le style "unread" de toutes les notifications
            document.querySelectorAll('.notification-item.unread').forEach(item => {
                item.classList.remove('unread');
            });
            updateNotificationsBadge();
